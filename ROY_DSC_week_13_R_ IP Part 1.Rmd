---
title: 'ROY MWANGI DSC week 13 IP with R: Part 1'
output:
...
```
observation: the dateset has 6 columns.
...
##3.2a Identifying the categorical class in the data 
```{r}
#Checking the data types of the columns
...
```
observation: As we can see, removing the highly correlated variables has  reduced multicollinearity in our dataset. It also made it easier to proceed with the modeling.
...
```
observation: there is no variable above correlation of 0.75. thus we can continue with the modeling.
...
```
the model gives us the accuracy of 92%. the model did a good job.


recommendation:

This being a classification problem, we recommend decision tree model, it gave good accuracy of over 90%. this is a good indication that the model do well where the target/label is binary. 




note: i just noticed there is a slight variance of output on HTML.
